...
mod tests {
    ...
    fn test_register_basic() {
        ...
        reg.write(0x7);
        assert_eq!(reg.read(), 0x7);
        
        reg.write(0xFF); // Should mask to 0xF
        assert_eq!(reg.read(), 0xF);
    }

    #[test]
    fn test_register_increment() {
        let mut reg = Register4Bit::with_value(0xE);
        
        reg.increment();
        assert_eq!(reg.read(), 0xF);
        
        reg.increment(); // Should wrap to 0
        assert_eq!(reg.read(), 0x0);
    }

    #[test]
    fn test_register_decrement() {
        let mut reg = Register4Bit::with_value(0x1);
        
        reg.decrement();
        assert_eq!(reg.read(), 0x0);
        
        reg.decrement(); // Should wrap to 0xF
        assert_eq!(reg.read(), 0xF);
    }
    ...
    fn test_cpu_registers() {
        ...
        regs.accumulator.write(0xA);
        regs.program_counter.write(0x5);
        regs.instruction_register.write(0x3);
        
        assert_eq!(regs.accumulator.read(), 0xA);
        assert_eq!(regs.program_counter.read(), 0x5);
        assert_eq!(regs.instruction_register.read(), 0x3);
    }
    ...
    fn test_flags() {
        ...
        regs.update_flags(0x0, false);
        assert!(regs.zero_flag);
        assert!(!regs.carry_flag);
        
        regs.update_flags(0x5, true);
        assert!(!regs.zero_flag);
        assert!(regs.carry_flag);
    }
}