...
mod tests {
    ...
    fn test_simple_assembly() {
        ...
        assert_eq!(result, vec![0xA, 0x5, 0xF]);
    }
    ...
    fn test_hex_numbers() {
        let source = "LDI 0xF\nADD 0xE";
        let result = assemble(source).unwrap();
        assert_eq!(result, vec![0xA, 0xF, 0x3, 0xE]);
    }
    ...
    #[test]
    fn test_disassembly() {
        let machine_code = vec![0xA, 0x5, 0x3, 0xE, 0xF];
        let disassembly = Assembler::disassemble(&machine_code);
        assert!(disassembly.contains("LDI"));
